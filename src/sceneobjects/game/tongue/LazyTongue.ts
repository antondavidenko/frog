...
export class LazyTongue implements ITongue {
  ...
  private addTonguePointer() {
    ...
    this.scene.input.on('drag', function (_pointer: Phaser.Input.Pointer, gameObject, dragX: number, dragY: number) {
      gameObject.x = dragX;
      gameObject.y = dragY;
    });
    if (this.isRollBackOnDragend) {
      this.scene.input.on('dragend', function (_pointer: Phaser.Input.Pointer, gameObject) {
